...
// Metadata drives the generic "add step" UI in panel.js — each op declares
// its param shape instead of panel.js hand-rolling a form per op.
// param.type: "keys" (multi) | "key" (single, dot-path ok) | "conditions" |
//             "text" | "number" | "select" | "code" (textarea)
export const OP_DEFINITIONS = [
  // --- array of objects ---
  { id: "extract", label: "Extract field(s)", category: "array", params: [{ name: "keys", type: "keys", label: "Fields" }] },
  { id: "filter", label: "Filter", category: "array", params: [{ name: "conditions", type: "conditions", label: "Conditions" }] },
  { id: "sort", label: "Sort", category: "array", params: [
  ...
  { id: "duplicates", label: "Find duplicates", category: "array", params: [{ name: "key", type: "key", label: "Field" }] },
  { id: "aggregate", label: "Aggregate", category: "array", params: [
    { name: "key", type: "key", label: "Field" },
    { name: "fn", type: "select", label: "Function", options: ops.AGGREGATE_FNS, default: "sum" },
  ] },
  { id: "slice", label: "Slice first/last N", category: "array", params: [
    { name: "n", type: "number", label: "N", default: 10 },
    { name: "from", type: "select", label: "From", options: ["first", "last"], default: "first" },
  ] },
  { id: "deepSearch", label: "Deep search", category: "array", params: [{ name: "text", type: "text", label: "Search text" }] },
  { id: "flattenArray", label: "Flatten nested array", category: "array", params: [{ name: "key", type: "key", label: "Field" }] },
  { id: "toCSV", label: "Convert to CSV", category: "array", params: [] },
  { id: "toDictionary", label: "Convert to dictionary", category: "array", params: [{ name: "key", type: "key", label: "Key field" }] },
  ...
  { id: "getByPath", label: "Get by path", category: "object", params: [{ name: "path", type: "text", label: "Dot path" }] },
  { id: "listKeys", label: "List all keys (deep)", category: "object", params: [] },

  // --- escape hatch (SPEC.md §2 — always present) ---
  { id: "custom", label: "Custom expression…", category: "custom", params: [{ name: "code", type: "code", label: "Expression" }] },
];

export const OP_DEFINITIONS_BY_ID = Object.fromEntries(OP_DEFINITIONS.map((d) => [d.id, d]));

const DEFAULT_HANDLERS = {
  extract: (data, s) => ops.extractFields(data, s.keys ?? []),
  filter: (data, s) => ops.filterRows(data, s.conditions ?? []),
  sort: (data, s) => ops.sortRows(data, s.key, s.direction),
  groupBy: (data, s) => ops.groupBy(data, s.key),
  countBy: (data, s) => ops.countBy(data, s.key),
  unique: (data, s) => ops.uniqueValues(data, s.key),
  duplicates: (data, s) => ops.findDuplicates(data, s.key),
  aggregate: (data, s) => ops.aggregate(data, s.key, s.fn),
  slice: (data, s) => ops.sliceRows(data, Number(s.n) || 0, s.from),
  deepSearch: (data, s) => ops.deepSearch(data, s.text),
  flattenArray: (data, s) => ops.flattenByKey(data, s.key),
  toCSV: (data) => ops.toCSV(data),
  toDictionary: (data, s) => ops.toDictionary(data, s.key),
  pick: (data, s) => ops.pickKeys(data, s.keys ?? []),
  omit: (data, s) => ops.omitKeys(data, s.keys ?? []),
  flattenObject: (data) => ops.flattenObject(data),
...
export async function runPipeline(initialData, steps, handlerOverrides = {}) {
  const handlers = { ...DEFAULT_HANDLERS, ...handlerOverrides };
  ...
  for (const step of steps) {
    ...
    try {
      ...
      stepResults.push({ step, error: err.message, broken: true });
      stopped = true;
    }
  }
...
export function detectKeys(data) {
