...
async function capture(page: Page, { url, body }: { url: string; body: string }) {
  await page.evaluate(
    ({ url, body }) => {
...
test.skip("combine: join two captured responses by key via the plain dropdown picker", async ({
  ...
  await rows
    .nth(0)
    .locator(".combine-input-select")
    .selectOption({ label: "https://api.example.com/api/v2/accounts (/api/v2/accounts)" });
  ...
  await rows
    .nth(1)
    .locator(".combine-input-select")
    .selectOption({ label: "https://api.example.com/api/v2/orders (/api/v2/orders)" });
