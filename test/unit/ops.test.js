...
test("sortRows: asc/desc, nulls last regardless of direction", () => {
  ...
  assert.deepEqual(ops.sortRows(withNulls, "v", "asc").map((r) => r.v), [1, 2, null]);
  assert.deepEqual(ops.sortRows(withNulls, "v", "desc").map((r) => r.v), [2, 1, null]);
  assert.deepEqual(ops.sortRows(accounts, "balance", "desc").map((r) => r.code), ["A3", "A1", "A2"]);
});
...
test("sliceRows: first and last N", () => {
  assert.deepEqual(ops.sliceRows(accounts, 2, "first").map((r) => r.code), ["A1", "A2"]);
  assert.deepEqual(ops.sliceRows(accounts, 2, "last").map((r) => r.code), ["A2", "A3"]);
});
...
test("runCustomExpression: statement-body form with explicit return", () => {
  const result = ops.runCustomExpression(accounts, "const active = data.filter(x => x.status === 'active'); return active.length;");
  assert.equal(result, 2);
});
